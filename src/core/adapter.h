...
class CAdapter: public CFunctionBlock {
  ...
  protected:
    ...
    struct genAdapterFBConnDataSizeTemplate {
        enum {
          value = genFBConnDataSizeTemplate<((ta_nNumEIs < ta_nNumEOs) ? ta_nNumEOs : ta_nNumEIs),
                   ((ta_nNumDIs < ta_nNumDOs) ? ta_nNumDOs : ta_nNumDIs),
                   ((ta_nNumDIs < ta_nNumDOs) ? ta_nNumDOs : ta_nNumDIs)>::value
        };
    };
