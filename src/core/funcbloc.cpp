...
CFunctionBlock::CFunctionBlock(CResource *pa_poSrcRes, const SFBInterfaceSpec *pa_pstInterfaceSpec, const CStringDictionary::TStringId pa_nInstanceNameId, TForteByte *pa_acFBConnData, TForteByte *pa_acFBVarsData) :
   mEOConns(0), m_apoDIConns(0), mDOConns(0),
    m_apoAdapters(0), m_poResource(pa_poSrcRes), m_aoDIs(0), m_aoDOs(0), m_nFBInstanceName(pa_nInstanceNameId), m_enFBState(e_IDLE), m_bDeletable(true){
...
void CFunctionBlock::sendOutputEvent(int pa_nEO){
  ...
  if(pa_nEO < m_pstInterfaceSpec->m_nNumEOs){
    ...
    if(forte::core::eActive != monitoringData.mBreakpointSet){
#endif //FORTE_SUPPORT_MONITORING
    ...
    getEOConUnchecked(pa_nEO)->triggerEvent(*m_poInvokingExecEnv);
      ...
      if(monitoringData.mBreakpointEnable){
        if(forte::core::eOnce == monitoringData.mBreakpointSet){ // if < 0 -> continue called
          monitoringData.mBreakpointSet = forte::core::eInactive;
        }
        else{
          monitoringData.mBreakpointSet = forte::core::eActive;
         return;
        }
        // Get Resource
...
bool CFunctionBlock::startEIBreakpoint(TEventID p_nEventId){
  ...
  if(p_nEventId < cg_unInvalidPortId){
    forte::core::SMonitorEvent::SMonitorEventData &monitorEventData(m_nEIMonitorCount[p_nEventId].mMonitorEventData[m_nEIMonitorCount[p_nEventId].mBufPos]);
...
bool CFunctionBlock::startEOBreakpoint(TEventID p_nEventId){
  CCriticalRegion criticalRegion(m_poResource->m_oResDataConSync);
  if(p_nEventId < cg_unInvalidPortId){
    forte::core::SMonitorEvent::SMonitorEventData &monitorEventData(m_nEOMonitorCount[p_nEventId].mMonitorEventData[m_nEOMonitorCount[p_nEventId].mBufPos]);
    monitorEventData.mBreakpointEnable = true;
