 ...
 *
 */
unsigned int BBBIO_ADCTSC_one_shoot_read(unsigned int chn_ID)
{
	unsigned int *reg_count = NULL;
	unsigned int *reg_data = NULL;
	unsigned int *reg_ctrl = NULL;
	unsigned int buf_data = 0;
	int FIFO_count = 0;
	struct ADCTSC_channel_struct *chn_ptr = &ADCTSC.channel[chn_ID];
	struct ADCTSC_FIFO_struct *FIFO_ptr = ADCTSC.FIFO;
	int i;

	/*Verify input params*/
	if(chn_ID < BBBIO_ADC_AIN0 || chn_ID > BBBIO_ADC_AIN6){
#ifdef BBBIO_LIB_DBG
		fprintf(stderr, "BBBIO_ADCTSC_channel_ctrl : argument error\n");
#endif
		return 1;
	...
	*reg_ctrl |= (CTRL_ENABLE | CTRL_STEP_ID_TAG);
	...
	while(ADCTSC.channel_en_var !=0) {
			...
			FIFO_count = *reg_count;
			if(FIFO_count > 0) {
				/* fetch data and clear FIFO */
				for(i = 0 ; i < FIFO_count ; i++) {
					buf_data = *reg_data;
					if(((buf_data >> 16) & 0xF) == chn_ID){
						if(chn_ptr->buffer_size >= 1) {
							*(chn_ptr->buffer_save_ptr) = buf_data & 0xFFF;
							chn_ptr->buffer_count = 1;
						}
					}
				}
				ADCTSC.channel_en_var = 0;
			}
			// switch to next FIFO
			FIFO_ptr = FIFO_ptr->next;
	}
